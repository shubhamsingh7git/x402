...
export default function PolicyGuardPage() {
  ...
  const maxPerTxAmount = useAppStore((state) => state.maxPerTxAmount)
  const setMaxPerTxAmount = useAppStore((state) => state.setMaxPerTxAmount)
  const dailyBudgetLimit = useAppStore((state) => state.dailyBudgetLimit)
  const setDailyBudgetLimit = useAppStore((state) => state.setDailyBudgetLimit)
  const maxTxPerMinute = useAppStore((state) => state.maxTxPerMinute)
  const setMaxTxPerMinute = useAppStore((state) => state.setMaxTxPerMinute)
  const merchants = useAppStore((state) => state.merchants)
  const addMerchant = useAppStore((state) => state.addMerchant)
  const deleteMerchant = useAppStore((state) => state.deleteMerchant)
        ...
        >
          <div className="flex items-center justify-between">
            ...
            <span className="text-base font-bold font-mono text-blue-500">
              ${Number(maxPerTxAmount ?? 0).toFixed(2)} USDC
            </span>
          </div>
        ...
        >
          <div className="flex items-center justify-between">
            ...
            <span className="text-base font-bold font-mono text-emerald-500">
              ${Number(dailyBudgetLimit ?? 0).toFixed(2)} USDC
            </span>
          </div>
      ...
      >
        <div className="p-6 border-b border-border/30 flex items-center justify-between">
          ...
          <Button
            ...
            variant="default"
            className="h-10 px-4 rounded-xl text-xs font-bold gap-2 shadow-md"
          >
            ...
            >
              ...
              <form onSubmit={handleCreateMerchant} className="space-y-4">
                ...
                <div className="pt-2 flex justify-end gap-2">
                  ...
                  <Button type="submit" variant="default" className="h-10 rounded-xl text-xs font-bold">
