...
export function TopNav() {
  ...
  const isCdpConnected = useAppStore((state) => state.isCdpConnected)
  const selectedNetwork = useAppStore((state) => state.selectedNetwork)
  const sessionWalletAddress = useAppStore((state) => state.sessionWalletAddress)
    ...
    const handleScroll = () => {
      if (!ticking) {
        window.requestAnimationFrame(() => {
          const isOverThreshold = main.scrollTop > 10
      ...
      <div className="flex items-center gap-2 sm:gap-3">
        ...
        <div className="relative" ref={notificationRef}>
          <Button
            ...
            aria-label={`Notifications${unreadNotifications ? ', has unread' : ''}`}
        ...
        <button
          ...
          className="flex items-center gap-2 px-3 py-1.5 rounded-xl bg-muted/30 border border-border hover:border-primary/40 active:scale-95 cursor-pointer group transition-all duration-200"
        >
