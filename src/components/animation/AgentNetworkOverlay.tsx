...
function Particles() {
  ...
  return (
    <points ref={ref}>
      ...
      <pointsMaterial size={0.06} color="#06b6d4" transparent opacity={0.35} sizeAttenuation depthWrite={false} blending={THREE.AdditiveBlending} />
    </points>
  )
}
...
function Title() {
  ...
  return (
    <group ref={ref} position={[0, 5.5, 0]}>
      <Label text="AI AGENT NETWORK" pos={[0, 0, 0]} color="#ffffff" size={0.5} />
      <Label text="CONNECTED · INTELLIGENT · AUTONOMOUS" pos={[0, -0.55, 0]} color="#475569" size={0.16} />
    </group>
  )
}
...
export function AgentNetworkOverlay({ onComplete }: AgentNetworkOverlayProps) {
  ...
  useEffect(() => {
    ...
    // Hold for 4s, then fade out and complete
  ...
  return (
    <div
      ...
      style={{ background: "#060a12", opacity: 0 }}
    >
      <Canvas
      ...
      >
        ...
        <Title />
        <Particles />
        <gridHelper args={[30, 30, "#0e2a3a", "#0a1520"]} position={[0, -2.5, 0]} />
      </Canvas>
