...
export function AgentPipelineMap({ activeExecution = false }: { activeExecution?: boolean }) {
  ...
  return (
    <div className="w-full h-full min-h-[460px] rounded-2xl bg-[#F8FAFC] dark:bg-[#121212] border border-border/40 overflow-hidden relative">
      <ReactFlow
        nodes={nodes}
        edges={edges}
      ...
      >
        ...
        <Controls className="!bg-white dark:!bg-[#1A1A1A] !border-border/40 !rounded-xl" />
        <MiniMap
          zoomable
          pannable
          className="!bg-white dark:!bg-[#1A1A1A] !border-border/40"
          nodeColor={(n) => {
